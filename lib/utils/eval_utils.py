...
import json
import numpy as np
from .data_utils import bbox_denormalize, cxcywh_to_x1y1x2y2, ctr_denorm
...
def compute_IOU(bbox_true, bbox_pred, format='xywh'):
    ...
    # print(f"true: {bbox_true}, pred: {bbox_pred}, xmin: {xmin}, xmax: {xmax}, ymin: {ymin}, ymax: {ymax}, w_inter: {w_inter}, h_inter: {h_inter}, intersection: {intersection}, union: {union}")
...
def eval_jaad_pie_cvae(input_traj, target_traj, cvae_all_dec_traj):
    ...
    # tiled_target_traj += (input_traj[...,:2] + input_traj[...,2:])/2
    # cvae_all_dec_traj += (input_traj[...,:2] + input_traj[...,2:])/2
    ...
    # tiled_target_traj = ctr_denorm(tiled_target_traj)
    # cvae_all_dec_traj = ctr_denorm(cvae_all_dec_traj)

    tiled_target_traj_xyxy = cxcywh_to_x1y1x2y2(tiled_target_traj)
    cvae_all_dec_traj_xyxy = cxcywh_to_x1y1x2y2(cvae_all_dec_traj)
    # tiled_target_traj_xyxy = tiled_target_traj
    # cvae_all_dec_traj_xyxy = cvae_all_dec_traj
    
    
    
    # convert bbox to ctr
    # tiled_target_traj_xyxy = tiled_target_traj_xyxy[..., :2] + (tiled_target_traj_xyxy[..., 2:] - tiled_target_traj_xyxy[..., :2])/2
    # cvae_all_dec_traj_xyxy = cvae_all_dec_traj_xyxy[..., :2] + (cvae_all_dec_traj_xyxy[..., 2:] - cvae_all_dec_traj_xyxy[..., :2])/2
    ...
    # tmp_FIOU = []
    # # pdb.set_trace()
    # for i in range(tiled_target_traj_xyxy.shape[0]):
        # tmp_AIOU = []
        # for j in range(tiled_target_traj_xyxy.shape[1]):
            # # pdb.set_trace()
            # group_20_iou = []
            # for k in range(tiled_target_traj_xyxy.shape[2]):
                # group_20_iou.append(compute_IOU(tiled_target_traj_xyxy[i,j,k,:], cvae_all_dec_traj_xyxy[i,j,k,:], format='x1y1x2y2'))
                # if (j == tiled_target_traj_xyxy.shape[1] - 1) and (k == tiled_target_traj_xyxy.shape[2] - 1):
                    # tmp_FIOU.append(group_20_iou[-1])
            # tmp_AIOU.append(np.mean(group_20_iou))
    # AIOU = np.mean(tmp_AIOU)
    # FIOU = np.mean(tmp_FIOU)
    return MSE_15, MSE_05, MSE_10, FMSE, CMSE, CFMSE #, AIOU, FIOU
