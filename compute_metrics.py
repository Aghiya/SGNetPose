    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if line.startswith("MSE"):
                # Split the line by commas to get each "metric: value" pair
                pairs = line.split(',')
                ...
                for pair in pairs:
                    ...
                    if metric not in metrics:
                        metrics[metric] = []
                    metrics[metric].append(value)  # Append values for the current metric
...
def process_files(directory, prefixes):
    ...
    for prefix in prefixes:
        ...
        prefix_metrics = {metric: [] for metric in ['MSE_05', 'MSE_10', 'MSE_15', 'FMSE', 'AIOU', 'FIOU', 'CMSE', 'CFMSE']}
        ...
        for file in files:
            ...
            file_metrics = extract_metrics_from_file(os.path.join(directory, file))
...
def plot_metrics(averaged_metrics, output_dir):
    ...
    for metric in metrics_list:
        ...
        # Save the chart
        plt.savefig(os.path.join(output_dir, f"{dataset}_{metric.lower()}.png"))
        plt.close()
        
    for pfx in pfx_list:
        with open(os.path.join(output_dir, f"{pfx}_avg_metrics.txt"), "w") as f:
            for metric in metrics_list:
                try:
                    avg_value = averaged_metrics[pfx][metric][-1]  # Last epoch's value
